#!/usr/bin/env bash
set -euo pipefail
# This script applies the Rook Ceph Cluster helm chart
echoerr() { echo "$@" 1>&2; }
...
# Check the cluster to ensure that there are nodes labeled with role=storage-node
# kubectl label nodes <your-node-name> role=storage-node
if ! kubectl get nodes -l role=storage-node -o name | grep -q 'node/'; then
    fatal "No nodes labeled with role=storage-node found. Please label your storage nodes before applying the Rook Ceph chart."
fi

# Ensure the rook-ceph namespace can run privileged pods
kubectl label namespace rook-ceph pod-security.kubernetes.io/enforce=privileged

helm upgrade --install --create-namespace --namespace rook-ceph rook-ceph-cluster rook-release/rook-ceph-cluster -f values.yaml
